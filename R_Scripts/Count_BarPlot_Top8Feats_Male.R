...
df = read.csv('Any2Common_inAll45Feats_Data_Male.csv')
df = df[,2:13]
df1 = df[df$APPRDX==1, ]
df2 = df[df$APPRDX==2, ]
...
write.csv(merged_counts, 'Counts_TopCommon11Feats_Male.csv')

########
df = read.csv("Counts_TopCommon11Feats_Male.csv",header = T)
...
for (col in columns) {
  ...
  print(p)
  ggsave(paste0(col,"_Percentage_Male.svg"))
}
